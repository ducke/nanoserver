...
<#
    ...
    .EXAMPLE
        New-NanoServerImage -MediaPath D:\ -BasePath .\Base -TargetPath '.\Target 1' -Compute -AdministratorPassword "Passw0rd" -ComputerName "NANO" -DomainName "ContosoDomain" -EnableIPDisplayOnBoot
...
Function Add-Package([String]$PackageFilePath, [Bool]$IsLanguage)
{
    ...
    Invoke-Command "& '$Script:BaseDismFilePath' /Add-Package /PackagePath:'$PackageFilePath' /Image:'$Script:TargetMountPath' /LogLevel:2 /LogPath:'$Script:DismLogFile'"
...
Function Add-Drivers()
{
    ...
    Invoke-Command "& '$Script:BaseDismFilePath' /Add-Driver /Driver:'$Script:DriversPath' /Recurse /Image:'$Script:TargetMountPath' /LogLevel:2 /LogPath:'$Script:DismLogFile'"
}
...
Function Add-ServicingDescriptor([String]$ComputerName, [String]$AdministratorPassword)
{
    ...
    Invoke-Command "& '$Script:BaseDismFilePath' /Image:'$Script:TargetMountPath' /Apply-Unattend:'$Script:TargetUnattendFilePath' /LogLevel:2 /LogPath:'$Script:DismLogFile'"
...
Function Join-Domain([String]$DomainName, [Bool]$ReuseDomainNode)
{
    ...
    if($Script:DomainBlobPath)
    {
        ...
        Invoke-Command "djoin /RequestODJ /LoadFile '$Script:DomainBlobPath' /WindowsPath '$Script:TargetMountPath\Windows'"
    }
}
...
Function Enable-Debug([String]$BCDPath)
{
    ...
    Invoke-Command("bcdedit /Store '$BCDPath' /BootDebug ``{bootmgr``} ON")
    Invoke-Command("bcdedit /Store '$BCDPath' /BootDebug ``{default``} ON")
    Invoke-Command("bcdedit /Store '$BCDPath' /Debug ``{default``} ON")
}

Function Enable-DebugSerial([String]$BCDPath, [Int]$Port, [Int]$BaudRate)
{
    Invoke-Command("bcdedit /Store '$BCDPath' /DBGSettings SERIAL DEBUGPORT:$Port BAUDRATE:$BaudRate")
}
...
Function Enable-DebugNet([String]$BCDPath, [String]$RemoteIP, [String]$RemotePort, [String]$Key)
{
    ...
    Invoke-Command("$Command > '$Script:TargetDebuggingKeyFilePath'")
...
{
    Invoke-Command("bcdedit /Store '$BCDPath' /DBGSettings 1394 Channel:$Channel")
}

Function Enable-DebugUSB([String]$BCDPath, [String]$TargetName)
{
    Invoke-Command("bcdedit /Store '$BCDPath' /DBGSettings USB TargetName:$TargetName")
}
...
Function Enable-EMS([String]$BCDPath, [Int]$Port, [Int]$BaudRate)
{
    ...
    Invoke-Command("bcdedit /Store '$BCDPath' /EMS ``{default``} ON")
    Invoke-Command("bcdedit /Store '$BCDPath' /EMSSettings EMSPORT:$Port EMSBAUDRATE:$BaudRate")
    Invoke-Command("bcdedit /Store '$BCDPath' /BootEMS ``{default``} ON")
    Invoke-Command("bcdedit /Store '$BCDPath' /BootEMS ``{bootmgr``} ON")
}
...
Function Mount-Image()
{
    ...
    Invoke-Command "& '$Script:BaseDismFilePath' /Mount-Image /ImageFile:'$Script:TargetVHDImageFilePath' /MountDir:'$Script:TargetMountPath' -Index:1 /LogLevel:2 /LogPath:'$Script:DismLogFile'"
}
...
Function Dismount-Image([Switch]$Discard)
{
    ...
    if($Discard)
    {
        Invoke-Command "& '$Script:BaseDismFilePath' /Unmount-Image /MountDir:'$Script:TargetMountPath' /Discard /LogLevel:2 /LogPath:'$Script:DismLogFile'"
    }
    else
    {
        Invoke-Command "& '$Script:BaseDismFilePath' /Unmount-Image /MountDir:'$Script:TargetMountPath' /Commit /LogLevel:2 /LogPath:'$Script:DismLogFile'"
    }
}
